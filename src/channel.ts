...
> {
   ...
   *
   * @param {StreamChat<StreamChatGenericse>} client the chat client
  ...
  async sendEvent(event: Event<StreamChatGenerics>) {
    ...
    return await this.getClient().post<EventAPIResponse<StreamChatGenerics>>(
  ...
  async search(
    ...
    return await this.getClient().get<SearchAPIResponse<StreamChatGenerics>>(
   ...
   * @param {UserSort<StreamChatGenerics>} [sort] Sort options, for instance [{created_at: -1}].
  ...
  async update(
    channelData:
      | Partial<ChannelData<StreamChatGenerics>>
      | Partial<ChannelResponse<StreamChatGenerics>> = {},
  ...
  async updatePartial(update: PartialUpdateChannel<StreamChatGenerics>) {
    return await this.getClient().patch<
      PartialUpdateChannelAPIResponse<StreamChatGenerics>
  ...
  async enableSlowMode(coolDownInterval: number) {
    const data = await this.getClient().post<
      UpdateChannelAPIResponse<StreamChatGenerics>
    >(this._channelURL(), {
   ...
   */
  async delete() {
    return await this.getClient().delete<DeleteChannelAPIResponse<StreamChatGenerics>>(
      this._channelURL(),
      {},
    );
  }
   ...
   * @return {Promise<UpdateChannelAPIResponse<StreamChatGenerics>>} The server response
   */
  async addModerators(members: string[], message?: Message<StreamChatGenerics>) {
    return await this._update({
   ...
   * @return {Promise<UpdateChannelAPIResponse<StreamChatGenerics>>} The server response
   */
  async removeMembers(members: string[], message?: Message<StreamChatGenerics>) {
    return await this._update({
   ...
   * @return {Promise<UpdateChannelAPIResponse<StreamChatGenerics>>} The server response
   */
  async demoteModerators(members: string[], message?: Message<StreamChatGenerics>) {
    return await this._update({
  ...
  getMessagesById(messageIds: string[]) {
    return this.getClient().get<GetMultipleMessagesAPIResponse<StreamChatGenerics>>(
  ...
  async query(options: ChannelQueryOptions<StreamChatGenerics>) {
    ...
    const state = await this.getClient().post<ChannelAPIResponse<StreamChatGenerics>>(
      queryURL + '/query',
      {
        data: this._data,
        state: true,
